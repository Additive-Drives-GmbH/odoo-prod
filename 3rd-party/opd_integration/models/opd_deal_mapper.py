...
class DealMapper(models.Model):
    """
        LeadMapper Model
        This model represents a mapping between deal fields in Odoo and Pipedrive.
    """
    _name = "opd.dealmapper"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Deal Mapper"
    ...
    @api.model
    def fetch_and_store_deal_fields(self):
        return self.env['opd.mapper.mixin'].fetch_and_store_fields(
            'crm_lead','dealFields',self.env['opd.dealmapper'],'deal',
            'dealmapper_id', 'pipedriveinstance.deals.lines')
